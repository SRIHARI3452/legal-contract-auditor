...
AMBIGUOUS TERMS: "reasonable time", "promptly", "best efforts" — replace with specific numbers.

MISSING CLAUSES: Confidentiality/NDA, Dispute resolution, Signature blocks, Effective Date.

RISKY LANGUAGE: Blanket liability exclusions, termination without notice, weak protection standards,
prior knowledge exclusions without burden of proof, contractor disclosure without written NDA.

CONTRADICTIONS: Conflicting payment terms, interest rates, IP ownership, SLA vs disclaimers,
governing law vs venue, conflicting time periods.

UNDEFINED REFERENCES: "Schedule A", "Exhibit A" referenced but not attached.

COMPLIANCE RISKS: Unrestricted third-party data sharing violating GDPR/CCPA/DPA.

═══ STRATEGY ═══
1. read_contract first
2. identify_issue with detailed description
3. apply_fix immediately after each issue (fix_text MUST be 80+ characters)
4. submit_audit after ALL issues found and fixed
5. Never submit false positives
"""
    ...
    for attempt in range(retries):
        try:
            ...
            if "Rate limit" in err_str or "429" in err_str:
                print(
                    f"  [Rate Limit] Throttled. Sleeping 10s... (attempt {attempt+1}/{retries})",
                    file=sys.stderr,
                )
                time.sleep(10)
            else:
...
def build_step_context(step_num: int, obs_dict: Dict[str, Any]) -> str:
    ...
    if step_num == 1:
        ...
        parts.append("\nGood progress! Consider submitting your audit if you have found all issues.")
...
def run_episode(client: OpenAI, task_id: str) -> Dict[str, Any]:
    ...
    try:
        resp = requests.post(f"{ENV_URL}/reset", json={"task_id": task_id}, timeout=60)
...
def main() -> None:
    ...
    print(f"  API Key:   SET", file=sys.stderr)
